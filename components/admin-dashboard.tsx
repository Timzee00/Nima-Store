...
import{ImagePlus,LogOut,PenLine,Plus,RefreshCw,Trash2,X}from"lucide-react";
...
export function AdminDashboard({initialProducts,initialOrders,stats}:{initialProducts:Product[];initialOrders:Order[];stats:{products:number;orders:number;sales:number;lowStock:number}}){
 const[products,setProducts]=useState(initialProducts),[orders,setOrders]=useState(initialOrders),[totals,setTotals]=useState(stats),[modal,setModal]=useState<Product|null|false>(false),[toast,setToast]=useState("");
 const flash=(x:string)=>{setToast(x);setTimeout(()=>setToast(""),2200)};
 async function refresh(){location.reload()}
 async function remove(id:string){if(!confirm("Archive this product? It will disappear from the storefront."))return;const r=await fetch("/api/admin/products?id="+encodeURIComponent(id),{method:"DELETE"});if(r.ok){const p=products.find(x=>x.id===id);setProducts(products.map(x=>x.id===id?{...x,active:false}:x));setTotals({...totals,lowStock:Math.max(0,totals.lowStock-((p?.stock??6)<=5?1:0))});flash("Product archived.")}}
 async function changeStatus(id:string,value:string){const r=await fetch("/api/admin/orders",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status:value})});if(r.ok){setOrders(orders.map(o=>o.id===id?{...o,status:value}:o));flash("Order updated.")}}
 return <div className="admin-shell"><div className="admin-top"><div><strong>NIMA.</strong><span style={{opacity:.45,marginLeft:10}}>Control room</span></div><div style={{display:"flex",gap:8}}><ThemeToggle/><button className="icon-btn" style={{borderColor:"rgba(255,255,255,.2)",color:"#fff"}} onClick={refresh} aria-label="Refresh"><RefreshCw size={17}/></button><a className="btn secondary" style={{borderColor:"rgba(255,255,255,.2)",color:"#fff"}} href="/admin/support">Support</a><a className="btn secondary" style={{borderColor:"rgba(255,255,255,.2)",color:"#fff"}} href="/" target="_blank">View store</a><button className="icon-btn" style={{borderColor:"rgba(255,255,255,.2)",color:"#fff"}} onClick={async()=>{await fetch("/api/admin/logout",{method:"POST"});location.href="/admin/login"}} aria-label="Log out"><LogOut size={16}/></button></div></div><main className="admin-main"><div style={{marginBottom:24}}><div className="eyebrow">Store overview</div><h1 style={{fontSize:"clamp(38px,5vw,62px)",letterSpacing:"-.06em",margin:"5px 0 0"}}>Run the shop.</h1></div><div className="stats"><div className="stat"><span>Products</span><strong>{totals.products}</strong></div><div className="stat"><span>Orders</span><strong>{totals.orders}</strong></div><div className="stat"><span>Recorded sales</span><strong>{money(totals.sales)}</strong></div><div className="stat"><span>Low stock</span><strong>{totals.lowStock}</strong></div></div><div className="admin-grid"><section className="admin-card"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:16}}><h2 style={{margin:0}}>Inventory</h2><button className="btn" onClick={()=>setModal(null)}><Plus size={16}/> Add product</button></div><div className="table-wrap"><table className="table"><thead><tr><th>Product</th><th>Category</th><th>Price</th><th>Stock</th><th>Actions</th></tr></thead><tbody>{products.filter(p=>p.active).map(p=><tr key={p.id}><td><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:38,height:46,borderRadius:9,overflow:"hidden",position:"relative",background:"#eee"}}>{p.images[0]&&<Image src={p.images[0]} alt="" fill sizes="38px" unoptimized style={{objectFit:"cover"}}/>}</div><div><strong>{p.name}</strong><div style={{fontSize:11,color:"var(--muted)"}}>{p.images.length} image{p.images.length===1?"":"s"} · Live</div></div></div></td><td>{p.category}</td><td>{money(p.salePrice??p.price)}</td><td>{p.stock}</td><td><div style={{display:"flex",gap:6}}><button className="icon-btn" style={{width:34,height:34}} onClick={()=>setModal(p)} aria-label={"Edit "+p.name}><PenLine size={14}/></button><button className="icon-btn" style={{width:34,height:34}} onClick={()=>remove(p.id)} aria-label={"Archive "+p.name}><Trash2 size={14}/></button></div></td></tr>)}</tbody></table></div></section><section className="admin-card"><h2>Recent orders</h2><div className="table-wrap"><table className="table"><thead><tr><th>Customer</th><th>Total</th><th>Status</th></tr></thead><tbody>{orders.map(o=><tr key={o.id}><td><strong>{o.customerName}</strong><div style={{fontSize:11,color:"var(--muted)"}}>{o.phone}</div><div style={{fontSize:11,color:"var(--muted)",maxWidth:190,overflow:"hidden",textOverflow:"ellipsis"}}>{o.deliveryAddress}</div></td><td>{money(o.total)}</td><td><select className="status" value={o.status} onChange={e=>changeStatus(o.id,e.target.value)} aria-label="Order status"><option value="pending">Pending</option><option value="confirmed">Confirmed</option><option value="fulfilled">Fulfilled</option><option value="cancelled">Cancelled</option></select></td></tr>)}</tbody></table></div></section></div></main>{modal!==false&&<ProductModal existing={modal||undefined} close={()=>setModal(false)} onSaved={p=>{setProducts(cur=>modal?cur.map(x=>x.id===p.id?p:x):[p,...cur]);setTotals({...totals,products:modal?totals.products:totals.products+1});setModal(false);flash(modal?"Product updated.":"Product added.")}}/>}<div className={"toast "+(toast?"show":"")}>{toast}</div></div>
}
